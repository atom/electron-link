...
function isReferenceToShadowedVariable (path) {
  const referenceLocation = Point.fromLocation(path.node.loc.start)
  const variableRedeclarations = path.scope.getBindings()[path.node.name] || []
  const variableRedeclarationLocations = variableRedeclarations.map(b => Point.fromLocation(b.node.loc.start))
  return variableRedeclarationLocations.some(l => l.compare(referenceLocation) < 0)
}
