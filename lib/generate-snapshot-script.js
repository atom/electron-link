...
module.exports = function generateSnapshotScript (options) {
  ...
  while (requiredModulePaths.length > 0) {
    ...
    if (!moduleASTs[relativeFilePath]) {
      ...
      const ast = processRequires({filePath, source, baseDirPath: options.baseDirPath, didFindRequire: modulePath => {
