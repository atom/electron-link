...
module.exports = function (options) {
    ...
    if (astPath.scope.isGlobal) {
      ...
      while (astPath) {
        ...
        if (node.type === 'FunctionExpression') {
          if (astPath.parent.node.type === 'CallExpression' || astPath.parent.parent.node.type === 'CallExpression') {
            return true
          }
        }
  ...
  function resolveModulePath (moduleName) {
    try {
      return resolve.sync(moduleName, {basedir: path.dirname(options.filePath), extensions: ['.js', '.json']})
    } catch (e) {
      return null
    }
  }
